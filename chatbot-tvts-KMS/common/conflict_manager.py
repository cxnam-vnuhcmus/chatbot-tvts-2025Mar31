...
class ConflictManager:
    ...
    def sync_group_conflicts_by_group(self, group_id):
        ...
        try:
            ...
            # If documents found in group, get ID of first document to sync
            first_doc_id = group_docs[0]['id']
            return self.sync_group_conflicts(first_doc_id)
    ...
    def analyze_document_bak(self, doc_id: str) -> Dict:
        """
    ...
    def analyze_content_conflicts(self, chunks: List[Dict]) -> List[ConflictResult]:
        ...
        # First filter out disabled chunks
        enabled_chunks = []
        disabled_chunks = []
        for chunk in chunks:
            # Properly check if chunk is disabled
            is_enabled = True
        ...
        for index, chunk in enumerate(enabled_chunks):
            try:
                ...
                logger.error(f"Error analyzing content for chunk {index}: {str(e)}")
                logger.error(traceback.format_exc())
        
        # Handle previously detected conflicts for disabled chunks
        if doc_id and disabled_chunks:
            try:
                # Get existing conflicts from database for disabled chunks
    ...
    def handle_conflicts(self, chunk_id: str, db_manager=None):
        ...
        try:
            ...
            # Get existing conflicts for this chunk
            query = """
    ...
    def analyze_external_conflicts(self, doc_id, chunks):
        ...
        try:
            related_docs = self._get_all_documents_except(doc_id)
            if not related_docs:
                logger.info(f"No other documents found to compare with {doc_id}")
                return []
                
            logger.info(f"Found {len(related_docs)} other docs to compare with {doc_id}")
        except Exception as e:
            logger.error(f"Error retrieving related document: {str(e)}")
            return []
        ...
        for related_doc in related_docs:
            ...
            try:
                ...
                for chunk1 in enabled_chunks:
                    chunk1_id = chunk1['id']
                    chunk1_doc_id = doc_id 
                    
                    for chunk2 in enabled_related_chunks:
                        chunk2_id = chunk2['id']
                        chunk2_doc_id = related_doc_id 
                        
                        if chunk1_doc_id == chunk2_doc_id:
                            logger.info(f"Skip internal conflict analysis between {chunk1_id} and {chunk2_id} because same document")
                            continue
                        
                        sorted_ids = sorted([chunk1_id, chunk2_id])
                        cache_key = f"external_{sorted_ids[0]}_{sorted_ids[1]}"
                        
                        cached_result = self._get_from_cache(cache_key)
                        if cached_result:
                            if cached_result.has_conflict:
                                logger.info(f"Detected conflict from cache: {cache_key}")
                                conflicts.append(cached_result.to_dict())
                            continue
                        
                        logger.info(f"Analyze external conflicts between {chunk1_id} ({chunk1_doc_id}) and {chunk2_id} ({chunk2_doc_id})")
                        
                        result = self.analyzer.analyze_chunks([
                            (chunk1['id'], chunk1.get('original_text', '')),
                            (chunk2['id'], chunk2.get('original_text', ''))
                        ...
                        if result.has_conflict:
                            ...
                            conflicts.append(result.to_dict())
                            ...
                            try:
                                ...
                                self.db.update_document_status(doc_id, conflict_data)
                                self.db.update_document_status(related_doc_id, conflict_data)
                                
                                logger.info(f"Conflict saved for both documents: {doc_id} and {related_doc_id}")
                                
                            except Exception as store_error:
                                logger.error(f"Contradictory save error: {str(store_error)}")
                                logger.error(traceback.format_exc())
                                
            except Exception as e:
                logger.error(f"Error while parsing conflicting document {related_doc_id}: {str(e)}")
    ...
    # def sync_group_conflicts(self, doc_id: str) -> Dict:
    #     try:
    #         document = self.db.get_document_by_id(doc_id)
    #         if not document:
    ...
    #             logger.info(f"Document {doc_id} does not belong to any duplicate groups")
    #             return {
    #                 'conflicts': [],
    #                 'status': "No Conflict"
    #             }

    #         logger.info(f"Fetching documents in group {duplicate_group_id}")
    #         group_docs = self.db.get_documents_in_group(duplicate_group_id)
    #         if not group_docs:
    #             logger.warning(f"No documents found in group {duplicate_group_id}")
    #             return {
    #                 'conflicts': [],
    #                 'status': "No Conflict"
    #             }
    ...
    #                     # Check if all chunks involved in this conflict are enabled
    ...
    #                     conflict_data = {
    #                         "chunk_ids": chunk_ids,
    #                         "explanation": explanation or "Mâu thuẫn được phát hiện",
    ...
    #                             if content_key not in processed_content_keys:
    ...
    #                         if internal_key not in processed_internal_keys:
    ...
    #                         if external_key not in processed_external_keys:
    #                             processed_external_keys.add(external_key)
    #                             all_external_conflicts.append(conflict_data)
    #         except Exception as db_error:
    #             logger.error(f"Error when querying conflicting statements: {str(db_error)}")
    #             logger.error(traceback.format_exc())
    ...
    #                         if chunk_id:
    #                             # Check if this chunk is enabled
    #                             chunk_doc_id = chunk_id.split('_paragraph_')[0] if '_paragraph_' in chunk_id else None
    #                             if chunk_doc_id and chunk_doc_id in enabled_chunks_by_id:
    #                                 if chunk_id not in enabled_chunks_by_id[chunk_doc_id]:
    ...
    #                         # Check if all chunks involved are enabled
    #                         all_chunks_enabled = True
    #                         for chunk_id in chunk_ids:
    #                             chunk_doc_id = chunk_id.split('_paragraph_')[0] if '_paragraph_' in chunk_id else None
    #                             if chunk_doc_id and chunk_doc_id in enabled_chunks_by_id:
    ...
    #         logger.info(f"Collected conflict information: {len(all_content_conflicts)} content, "
    #             f"{len(all_internal_conflicts)} internal, {len(all_external_conflicts)} external")
    ...
    #                 logger.info(f"Synchronizing conflicting information for document {doc['id']}")
    #                 self.db.update_document_status(doc['id'], {
    #                     'has_conflicts': has_conflicts,
    ...
    #                 logger.error(f"Error processing document in group: {str(doc_error)}")
    ...
    #     except Exception as e:
    #         logger.error(f"Error in sync_group_conflicts: {str(e)}")
    #         logger.error(traceback.format_exc())
    #         return {
    #             'conflicts': [],
    #             'status': "No Conflict"
    ...
    def sync_group_conflicts(self, doc_id: str) -> Dict:
        """
        Synchronize conflict information for all documents in a duplicate group based on input document id
        
        Args:
            doc_id (str): ID of document to sync conflicts for
        ...
        try:
            ...
            logger.info(f"Getting documents in group {duplicate_group_id}")
            group_docs = self.db.get_documents_in_group(duplicate_group_id)
            if not group_docs:
                logger.warning(f"No documents found in group {duplicate_group_id}")
                return {
            ...
            # First retrieve direct conflicts from database
            try:
                direct_conflicts_query = """
                    SELECT doc_id, conflict_id, chunk_ids, explanation, conflicting_parts, 
                        detected_at, conflict_type, resolved, severity
            ...
            # Generate a comprehensive conflict information object
            combined_conflict_info = {
                "content_conflicts": all_content_conflicts,
                "internal_conflicts": all_internal_conflicts,
                "external_conflicts": all_external_conflicts,
            ...
            logger.info(f"Updated conflict information for {updated_docs} documents in group {duplicate_group_id}")
            return {
                'conflicts': all_content_conflicts + all_internal_conflicts + all_external_conflicts,
                'status': conflict_status
            }
